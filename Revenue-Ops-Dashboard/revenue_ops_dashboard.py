...
with tabs[1]:
    if not st.session_state.done: st.info("Complete Setup and click Run Full Analysis to generate your health report.")
        ...
        for mod_name in ms:
            ...
            drag_val = d["drag"]
            if d["blended"] < 30: status_label = f'Critical ({d["blended"]}/100)'; status_co = "#dc2626"
            elif d["blended"] < 50: status_label = f'Needs work ({d["blended"]}/100)'; status_co = "#ea580c"
            elif d["blended"] < 70: status_label = f'Improving ({d["blended"]}/100)'; status_co = "#f59e0b"
            else: status_label = f'Good ({d["blended"]}/100)'; status_co = "#16a34a"
            drag_text = f'−{drag_val} pts process drag' if drag_val > 0 else 'No process drag'
            ...
            st.markdown(f'''<div style="display:flex;align-items:center;gap:14px;padding:10px 8px;border-bottom:1px solid #f1f5f9;background:{bar_bg};border-radius:8px;margin-bottom:4px">
                ...
                <div style="width:180px;flex-shrink:0;text-align:right">
                    <div style="font-family:JetBrains Mono;font-size:0.75rem;font-weight:700;color:{status_co}">{status_label}</div>
                    <div style="font-size:0.62rem;color:#475569;margin-top:1px">{drag_text}</div>
                </div>
            </div>''',unsafe_allow_html=True)
        ...
        with st.expander("📐 How are these scores calculated?", expanded=False):
            st.markdown(f'''<div style="font-size:0.82rem;color:#334155;line-height:1.7">
                <div style="font-weight:600;margin-bottom:0.5rem">Each module's blended score = (Data Score × 60%) + (Maturity Score × 40%)</div>
                <div style="font-size:0.78rem;color:#64748b;margin-bottom:0.75rem">Data score is calculated from your uploaded CSV metrics. Maturity score comes from your maturity assessment CSV (15 questions, Level 0–3). Process drag = Data Score − Blended Score.</div>
                <table class="cp-table">
                    <tr><th style="text-align:left">Module</th><th style="text-align:left">Data Score Formula</th><th style="text-align:center">Data</th><th style="text-align:center">Maturity</th><th style="text-align:center">Blended</th></tr>
                    <tr><td>📦 Demand Forecasting</td><td style="font-size:0.75rem;color:#64748b">Accuracy × 0.7 + (20 − |Bias|) × 1.5</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600">{drags["Demand Forecasting"]["data"]}</td><td style="text-align:center;font-family:JetBrains Mono">{ds.get("Demand Forecasting",50)}</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600;color:{scolor(drags["Demand Forecasting"]["blended"])}">{drags["Demand Forecasting"]["blended"]}</td></tr>
                    <tr><td>📋 Order Management</td><td style="font-size:0.75rem;color:#64748b">100 − Errors×5 − Disputes×3 − Amendments×0.5</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600">{drags["Order Management"]["data"]}</td><td style="text-align:center;font-family:JetBrains Mono">{ds.get("Order Management",50)}</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600;color:{scolor(drags["Order Management"]["blended"])}">{drags["Order Management"]["blended"]}</td></tr>
                    <tr><td>🚚 Order Fulfilment & Logistics</td><td style="font-size:0.75rem;color:#64748b">OTIF × 0.8 + (10 − Return Rate) × 2</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600">{drags["Order Fulfilment & Logistics"]["data"]}</td><td style="text-align:center;font-family:JetBrains Mono">{ds.get("Order Fulfilment & Logistics",50)}</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600;color:{scolor(drags["Order Fulfilment & Logistics"]["blended"])}">{drags["Order Fulfilment & Logistics"]["blended"]}</td></tr>
                    <tr><td>💰 Billing & Revenue Mgmt</td><td style="font-size:0.75rem;color:#64748b">100 − Errors×8 − Disputes×4</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600">{drags["Billing & Revenue Mgmt"]["data"]}</td><td style="text-align:center;font-family:JetBrains Mono">{ds.get("Billing & Revenue Mgmt",50)}</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600;color:{scolor(drags["Billing & Revenue Mgmt"]["blended"])}">{drags["Billing & Revenue Mgmt"]["blended"]}</td></tr>
                    <tr><td>🏦 Post-Sales & Financial Closure</td><td style="font-size:0.75rem;color:#64748b">Working capital health: CCC vs benchmark</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600">{drags["Post-Sales & Financial Closure"]["data"]}</td><td style="text-align:center;font-family:JetBrains Mono">{ds.get("Post-Sales & Financial Closure",50)}</td><td style="text-align:center;font-family:JetBrains Mono;font-weight:600;color:{scolor(drags["Post-Sales & Financial Closure"]["blended"])}">{drags["Post-Sales & Financial Closure"]["blended"]}</td></tr>
                </table>
            </div>''',unsafe_allow_html=True)
        st.markdown("</div>",unsafe_allow_html=True)
